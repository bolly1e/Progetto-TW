import { User, Day, Today, Evento, Attivita, Week } from "./Calendario.js";
...
async function searchUser(name) {
  ...
  try {
    await client.connect();
    let tmp = await coll.findOne({ name: name });
    if (tmp) return true;
    else return false;
  } finally {
    await client.close();
...
async function fetchWeek(name, day, w) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    week.days[i] = {
      ...
      y: date.getFullYear(),
      eventi:
        f.User.grid.decade[date.getFullYear() - 2024].year[date.getMonth() - 1]
          .month[date.getDate() - 1].eventi,
    };
...
server.set("views", "Calendario/views");

server.get("/Mese", (req, res) => {
  res.render("Mese");
});
...
server.post("/:user/act", async (req, res) => {
  ...
  console.log(await fetchAct(user, day));
});
...
const collAc = db.collection("Account");

async function addNameAndPswd(name, pswd) {
  //Aggiunge nome del nuovo utente in Account
  try {
    await client.connect();
    await collAc.insertOne({ name: name, pswd: pswd });
  } finally {
    await client.close();
  }
}

async function updatePswd(name, pswd) {
  //Aggiunge nome del nuovo utente in Account
  try {
    await client.connect();
    await collAc.updateOne({ name: name }, { $set: { pswd: pswd } });
  } finally {
    await client.close();
  }
}
...
async function updateActual(name) {
  ...
  try {
    await client.connect();
    await collAc.updateOne(
      { _id: "685d92199d2586992a6a8439" },
      { $set: { actual: name } }
    );
  } finally {
    await client.close();
...
async function verifyActual(name) {
  ...
  try {
    await client.connect();
    return await collAc.findOne({ actual: name });
  } finally {
    await client.close();
  }
}

async function fetchNamesAndPswd() {
  try {
    await client.connect();
    return await collAc.find().toArray();
  } finally {
    await client.close();
  }
}
...
server.get("/account", async (req, res) => {
  res.json({ names: await fetchNamesAndPswd() });
});
...
server.post("/account", async (req, res) => {
  ...
  let render = req.body.render;
  if (add) {
    await addNameAndPswd(name, pswd);
  }
  if (update) {
    await updateActual(actual);
  }
  if (reset) {
    await updatePswd(name, pswd);
  }
  if (render) {
    if (!(await searchUser(actual))) {
      await newUser(actual);
    }
  }
  if (req.body.comp) {
    res.send(await verifyActual(req.body.comp));
  }
  res.end();
});
