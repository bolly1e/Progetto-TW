...
export class Evento {
  constructor(event, inizio, fine){ //event e' una stringa
...
export class Month {
  ...
  SpaceLeft() { //ritorna quanti giorni aggiungere prima dell'inizio del mese
    let first = this.FirstMonday().d;
    return 8 - first;
  }
  SpaceRight() { //questa quanti alla fine 
    let first = this.FirstMonday().d;
    let tot = this.SpaceLeft() + this.len;
    return 42 - tot;
  }
  Prec() { //ritorna il mese precedente, utile per capire quali giorni mettere prima
    if (this.m > 1) {
      return new Month(this.m - 1, this.y);
    } else return new Month(12, this.y - 1);
  }
  Next() { //qui il mese successivo
    if (this.m < 12) {
      return new Month(this.m + 1, this.y);
    } else return new Month(1, this.y + 1);
  }
  Print() { //riempre l'array print con i giorni giusti
