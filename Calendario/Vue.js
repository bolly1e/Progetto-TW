...
//SICUREZZA
async function verifyActual(name) {
...
const general_month = Vue.createApp({
  data() {
    ...
    let thisMonth = new Month(ActualMonth(), ActualYear());
    ...
    return {
      ...
      today: Today(), //obj Day di oggi
      thisMonth: thisMonth, //obj Month di questo mese
      ActualMonth: ActualMonth(), //numero del mese, int non obj mese
      ActualYear: ActualYear(), //numero dell'anno
      monthNames: [
        "Gennaio",
        "Febbraio",
        "Marzo",
      ...
      // Parte visiva:
      showTodayEvents: false, //mostra eventi di oggi nella pagina principale
      showTodayNotes: false, //mostra note di oggi nella pagina principale
      showTodayAct: false, //mostra act di oggi nella pagina principale
      ...
      // Variabili dove caricare le risposte del server:
      week: week,
      eventi: day.eventi,
      note: "",
      act: day.act,
  ...
  methods: {
    changeTime() {
      ...
      this.today = new Day(d, m, y);
      this.ActualMonth = m;
      this.ActualYear = y;
      this.thisMonth = new Month(this.ActualMonth, this.ActualYear);
      this.thisMonth.Print();
    ...
    async loadTodayEvents(d, m, y) {
      try {
        await fetch(
          ...
          .then(async (response) => {
            this.TodayEvents = response.eventi;
          });
      } catch (error) {
        console.log("Errore: " + error);
      }
    },
    async loadTodayNotes(d, m, y) {
      try {
        await fetch(
          `http://${localhost}:3000/${user}/note?d=${d}&m=${m}&y=${y}`,
          {
            method: "GET",
            headers: {
              Accept: "application/json",
          ...
          .then(async (response) => {
            ...
            this.TodayNotes = response.note.slice(1, -1);
          });
      } catch (error) {
        console.log("Errore: " + error);
      }
    },
    async loadTodayAct(d, m, y) {
      try {
        await fetch(
          `http://${localhost}:3000/${user}/act?d=${d}&m=${m}&y=${y}`,
          {
            method: "GET",
            headers: {
              Accept: "application/json",
    ...
    async loadWeek(d, m, y) {
      try {
        await fetch(
          `http://${localhost}:3000/${user}/week?d=${d}&m=${m}&y=${y}&w=${this.week.w}`,
          {
            method: "GET",
            headers: {
              Accept: "application/json",
    ...
    async insertAct() {
      ...
      if (this.act == "" || this.act[5].i == "") {
        try {
          console.log(day1);
          await fetch(
            `http://${localhost}:3000/${user}/act?d=${day1.d}&m=${day1.m}&y=${day1.y}`,
            {
              method: "POST",
              headers: {
                Accept: "application/json",
